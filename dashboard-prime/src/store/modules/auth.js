...
const mutations = {
  setRestoringSession(state, value) {
    state.restoringSession = value
  },
...
const actions = {
  ...
  restoreSessionIfAvailable({ commit, dispatch, state, getters: gettersParam }) {
  ...
  configureSkillsClientForInception({ state, getters: gettersParam }) {
    return new Promise((resolve, reject) => {
      if (state.userInfo) {
