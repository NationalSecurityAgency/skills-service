...
describe('Client Display Accessibility tests', () => {
    ...
    runWithDarkMode.forEach((darkMode) => {
        it(`Initial View${darkMode}`, () => {
            ...
            cy.get('[data-cy="skillsDisplayHome"] [data-cy="achievementCelebrationMsg"]').should('be.visible')
        ...
        it(`skill with self reporting${darkMode}`, () => {
            cy.setDarkModeIfNeeded(darkMode)
            cy.cdVisit('/', true);
            cy.injectAxe();
            cy.contains('Overall Points');
        ...
        it(`skills search and filter${darkMode}`, () => {
            cy.setDarkModeIfNeeded(darkMode)
            cy.cdVisit('/', true);
            cy.injectAxe();
            cy.cdClickSubj(0, 'Subject 1', true);
