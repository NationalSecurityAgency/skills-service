...
describe('Generate Quiz Tests', () => {
    ...
    it('can not change textinput via AI question if grading is waiting', () => {
        cy.intercept('GET', '/public/config', (req) => {
            req.reply((res) => {
                const conf = res.body;
                conf.enableOpenAIIntegration = true;
        ...
        cy.get('[data-cy="disabledAiMsg"]').should('exist');
        cy.get('[data-cy="disabledAiMsg"]').should('have.text', 'The question type can not be changed as there are currently answers to this question waiting to be graded');
        cy.press('Escape')
        ...
        cy.get('[data-cy="aiButton"]').click()
        cy.get('[data-cy="disabledAiMsg"]').should('not.exist');
        cy.press('Escape')
