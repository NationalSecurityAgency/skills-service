...
describe('Self Report Approval History Tests', () => {
    ...
    it('approval history notes and rejection message should display truncated', () => {
        ...
        cy.get('[data-cy=selfReportApprovalHistoryTable] [data-label=Response]')
            .eq(0)
            .find('[data-cy=showMoreText] [data-cy=showMore]')
            .should('be.visible');
        ...
        cy.get('[data-cy=smtText]')
            .eq(0)
            .should('have.text', `${rejectMsg}`);
        cy.get('[data-cy=selfReportApprovalHistoryTable] [data-label=Response]')
            .eq(0)
            .find('[data-cy=showMoreText] [data-cy=showLess]')
            .click();
        cy.get('[data-cy=selfReportApprovalHistoryTable] [data-label=Response]')
            .eq(0)
            .find('[data-cy=showMoreText] [data-cy=showMore]')
        ...
        cy.get('[data-cy=selfReportApprovalHistoryTable] [data-label=Requested]')
            .eq(1)
            .find('[data-cy=showMoreText] [data-cy=showMore]')
            .should('be.visible');
        cy.get('[data-cy=smtText]')
            .eq(1)
            .should('have.text', `${requestMsg.substr(0, 50)}`);
        cy.get('[data-cy=selfReportApprovalHistoryTable] [data-label=Requested]')
            .eq(1)
            .find('[data-cy=showMoreText] [data-cy=showMore]')
            .click();
    ...
    it('sorting on the 2nd page+ should re-set paging', () => {
        ...
        cy.visit('/administrator/projects/proj1/self-report');
        cy.get(approvalHistoryTableSelector)
            .get('[data-cy=skillsBTablePaging]')
            .contains('2')
            .click();
        cy.get(approvalHistoryTableSelector)
            .contains('Loading')
    ...
    it('page size', () => {
        ...
        cy.visit('/administrator/projects/proj1/self-report');
        cy.get(`${approvalHistoryTableSelector} [data-cy="skillsBTablePageSize"]`)
            .select('10');
        cy.validateTable(approvalHistoryTableSelector, [
            [{
                colIndex: 0,
    ...
    it('approval should move the item to the history table', () => {
        ...
        cy.get('[data-cy="approvalSelect_user2-skill2"]')
            .click({ force: true });
        ...
        cy.get('[data-cy="approvalSelect_user5-skill2"]')
            .click({ force: true });
        cy.get('[data-cy="approveBtn"]')
            .click();
        cy.validateTable(approvalHistoryTableSelector, [
