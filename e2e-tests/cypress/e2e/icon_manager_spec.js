...
describe('Icon Manager Tests', () => {
    ...
    beforeEach(() => {
        ...
        cy.intercept({
            method: 'POST',
            url: '/admin/projects/proj1/subjects/subj1/skills/group1'
        }).as('saveGroup');
    ...
    it('group - select font awesome icon', () => {
        ...
        cy.get('.icon-item>button:visible', {timeout:10000}).should('be.visible').last().then(($el)=> {
            ...
            cy.get(`[data-cy="${clazz}"]`).should('have.length', '1').click({force:true});
            cy.get('[data-cy=saveDialogBtn]').scrollIntoView().should('be.visible').click();
            cy.get('.p-datatable-table-container').contains('Awesome Group 1').should('be.visible');
            const classes = clazz.split(' ');
            let iconClass = classes[classes.length-1];
            iconClass = iconClass.replace(/-link$/, '')
            cy.get(`i.${iconClass}`).should('be.visible');
    ...
    it('group - upload custom icon - persists on navigating away', () => {
        cy.createSubject(1, 1)
        cy.createSkillsGroup(1, 1, 1)
        cy.addSkillToGroup(1, 1, 1, 1)
        cy.addSkillToGroup(1, 1, 1, 2)
    ...
    it('group - upload custom icon - persists on refresh', () => {
        cy.createSubject(1, 1)
        cy.createSkillsGroup(1, 1, 1)
        cy.addSkillToGroup(1, 1, 1, 1)
        cy.addSkillToGroup(1, 1, 1, 2)
    ...
    it('group - warns before deleting the icon', () => {
        cy.createSubject(1, 1)
        cy.createSkillsGroup(1, 1, 1)
        cy.addSkillToGroup(1, 1, 1, 1)
        cy.addSkillToGroup(1, 1, 1, 2)
        ...
        cy.contains('Are you sure you want to delete valid_icon.png? This icon is currently used by: Awesome Group 1');
        cy.contains('Cancel').click();
    ...
    it('group - upload custom icon - persists when navigating from another project', () => {
        cy.request('POST', '/app/projects/proj2', {
            projectId: 'proj2',
            name: "proj2"
        })
    ...
    it('group - upload custom icon - persists when navigating from outside of admin', () => {
        cy.request('POST', '/app/projects/proj2', {
            projectId: 'proj2',
            name: "proj2"
        })
