...
describe('Skills Group Page Tests', () => {
  ...
  beforeEach(() => {
    ...
    Cypress.Commands.add('addSkillToGroupViaUI', (groupId, skillNum, expandGroupNum = 0) => {
      ...
      if (expandGroupNum !== null) {
        cy.get(`[data-p-index="${expandGroupNum}"] [data-pc-section="rowtogglebutton"]`).click()
      }
    ...
    cy.intercept('POST', '/api/projects/Inception/skills/ChangeSkillDisplayOrder').as('reportChangeSkillDisplayOrder')
  })
  const tableSelector = '[data-cy="skillsTable"]'

  it('create skills group and nav to group page', () => {
    ...
    // navigate to the skill group page
    cy.get('[data-cy="manageSkillLink_BlahGroup"]').click()
    cy.get('[data-cy="pageHeader"]').contains('GROUP: Blah')
  ...
  it('create multiple skills group and assign child to group 2', () => {
    ...
    // Navigate to group2 page and add skill
    cy.visit('/administrator/projects/proj1/subjects/subj1/groups/group2Group')
    cy.get('[data-cy="pageHeader"]').contains('GROUP: group2')
    cy.addSkillToGroupViaUI(`${group2}Group`, 1, null)
    
    // verify skill was added
    cy.validateTable(`[data-cy="ChildRowSkillGroupDisplay_group2Group"] [data-cy="skillsTable"]`, [
      [{ colIndex: 2, value: 'Skill 1' }],
    ], 5, true, null, false)

    // Navigate to group1 page to verify it's empty
    cy.visit('/administrator/projects/proj1/subjects/subj1/groups/group1Group')
    cy.get('[data-cy="pageHeader"]').contains('GROUP: group1')
    cy.get(`[data-cy="ChildRowSkillGroupDisplay_group1Group"] [data-cy="requiredAllSkills"]`).contains('all skills')
    cy.get(`[data-cy="ChildRowSkillGroupDisplay_group1Group"] [data-cy="noContent"]`)
  ...
  it('create group with description', () => {
    ...
    // Navigate to the group page to verify description
    cy.visit('/administrator/projects/proj1/subjects/subj1/groups/BlahGroup')
    cy.get('[data-cy="pageHeader"]').contains('GROUP: Blah')
  ...
  // This test doesn't apply to the new skill group page as it tests focus on subject page
  it('handle focus on edit', () => {
    ...
    cy.get('[data-cy="btn_edit-group"]').should('have.focus')
  })
  ...
  it('change display order for skills under a group', () => {
    ...
    cy.wait('@getSkill')
    cy.wait('@reportChangeSkillDisplayOrder')
    cy.validateTable(skillsTableSelector, [
      [{ colIndex: 2, value: 'Very Great Skill 5' }],
      [{ colIndex: 2, value: 'Very Great Skill 4' }],
      [{ colIndex: 2, value: 'Very Great Skill 6' }],
  ...
  it('total points in additional column are incremented when skills are added', () => {
    ...
    cy.visit('/administrator/projects/proj1/subjects/subj1/groups/group1')
    cy.wait('@getGroupSkills')
    cy.get('[data-cy="noContent"]').should('exist')
    cy.get('[data-cy="pageHeaderStat_Points"] [data-cy="statValue"]').should('have.text', '0');
    cy.addSkillToGroupViaUI('group1', 1, null)
  ...
  it('group overview cards are updated when group is enabled', () => {
    cy.createSkillsGroup(1, 1, 1)
    cy.addSkillToGroup(1, 1, 1, 1, { pointIncrement: 10, numPerformToCompletion: 5 })
    cy.addSkillToGroup(1, 1, 1, 2, { pointIncrement: 10, numPerformToCompletion: 5 })
    cy.addSkillToGroup(1, 1, 1, 3, { pointIncrement: 10, numPerformToCompletion: 5 })
  ...
  it('group overview cards are updated when skills added, deleted or modified for a group', () => {
    cy.createSkillsGroup(1, 1, 1)
    cy.addSkillToGroup(1, 1, 1, 1, { pointIncrement: 10, numPerformToCompletion: 5 })
    cy.addSkillToGroup(1, 1, 1, 2, { pointIncrement: 10, numPerformToCompletion: 5 })
    cy.addSkillToGroup(1, 1, 1, 3, { pointIncrement: 10, numPerformToCompletion: 5 })
  ...
  it('group overview cards are updated when group skill is copied', () => {
    cy.createSkillsGroup(1, 1, 1)
    cy.addSkillToGroup(1, 1, 1, 1, { pointIncrement: 10, numPerformToCompletion: 5 })
    cy.addSkillToGroup(1, 1, 1, 2, { pointIncrement: 10, numPerformToCompletion: 5 })
    cy.addSkillToGroup(1, 1, 1, 3, { pointIncrement: 10, numPerformToCompletion: 5 })
  ...
  it('search and navigate to group\'s skill', () => {
    ...
    cy.intercept('/admin/projects/proj1/skills?*skillNameQuery=skill*').as('searchSkills');
    ...
    cy.get('[data-cy="skillsSelector-skillName"]').should('have.length', 2).as('skillIds')
    cy.get('@skillIds').eq(0).contains('Skill 1')
    cy.get('@skillIds').eq(1).contains('Skill 2')
  ...
  it('search and navigate to group', () => {
    ...
    cy.get('[data-cy="skillsSelector"]').click()
    cy.get('li.p-autocomplete-empty-message').contains('Type to search for skills').should('be.visible')
    cy.intercept('/admin/projects/proj1/skills?*skillNameQuery=awesome*').as('searchSkills');
    cy.get(`[data-cy="skillsSelector"]`).type('awesome')
    cy.wait('@searchSkills')

    cy.get('[data-cy="skillsSelector-skillName"]').should('have.length', 1).as('skillIds')
    cy.get('@skillIds').eq(0).contains('Group 1')
