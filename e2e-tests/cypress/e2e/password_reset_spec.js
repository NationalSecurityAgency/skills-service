...
describe('Password Reset Tests', () => {
    ...
    it('reset password', () => {
        ...
        cy.getLinkFromEmail()
            .then((resetLink) => {
                cy.visit(resetLink);
                cy.get('[data-cy=resetPasswordSubmit]')
                    .should('exist');
                cy.get('[data-cy=resetPasswordEmail]')
                    .type('test@skills.org');
                cy.get('[data-cy=resetPasswordNewPassword]')
                    .type('password2');
                cy.get('[data-cy=resetPasswordConfirm]')
                    .type('password2');
                cy.get('[data-cy=resetPasswordSubmit]')
                    .click();

                cy.wait('@performReset');
                cy.get('[data-cy=resetConfirmation]')
                    .should('exist');
    ...
    it('reset not enabled if required configurations not set', () => {
        ...
        cy.logout();
        cy.intercept('GET', '/public/isFeatureSupported?feature=passwordreset')
            .as('isEnabled');
        cy.visit('/administrator/');
        cy.get('[data-cy=forgotPassword]')
            .click();
        cy.wait('@isEnabled');
        cy.get('[data-cy=resetNotSupported]')
            .should('be.visible');
        cy.get('[data-cy=forgotPasswordEmail')
            .should('have.length.lte', 0);
    });
});
