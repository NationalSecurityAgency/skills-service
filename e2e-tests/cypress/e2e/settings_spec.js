...
describe('Settings Tests', () => {
    ...
    it('custom header/custom footer must be 3000 characters or less', () => {
        ...
        cy.get('[data-cy=customFooter]')
            .clear()
            .fill(_3001);
        cy.get('#customHeader').then(($input) => {
            const text = $input.val();
            const textLength = text.length;

            expect(textLength).to.be.equal(3000);
        })
        cy.get('[data-cy="customHeaderNumCharsRemaining"]').should('have.text', '0 characters remaining ')

        cy.get('#customFooter').then(($input) => {
            const text = $input.val();
            const textLength = text.length;

            expect(textLength).to.be.equal(3000);
        })
        cy.get('[data-cy="customFooterNumCharsRemaining"]').should('have.text', '0 characters remaining ')
        cy.get('[data-cy=saveSystemSettings]')
            .should('not.be.disabled');
    });
