...
describe('Client Display Tests', () => {
  ...
  it('Point History\'s open menu must respect tiles.background option', () => {
    ...
    cy.request('POST', `/api/projects/proj1/skills/skill1`, {
      userId: Cypress.env('proxyUser'),
      timestamp: m.subtract(4, 'day').format('x')
    })
  ...
  it('skills group', () => {
    ...
    cy.cdVisit('/?enableTheme=true');
    cy.cdClickSubj(0);
    cy.get('[data-cy=skillCompletedCheck-group1').should('not.exist') // completed checkbox should not exists
  ...
  it('skills group - 1 out 2 skills required', () => {
    ...
    cy.cdVisit('/?enableTheme=true');
    cy.cdClickSubj(0);
    cy.get('[data-cy=skillCompletedCheck-group1').should('exist') // completed checkbox exists
