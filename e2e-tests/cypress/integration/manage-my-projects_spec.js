...
describe('Manage My Projects Tests', () => {
  ...
  beforeEach(() => {
    ...
    Cypress.Commands.add("addToMyProjectsViaBtn", (rowNum, expectedNumRows = 6) => {
      ...
      cy.get(rowSelector).should('have.length', expectedNumRows).as('cyRows');
      cy.get('@cyRows').eq(rowNum).find('td').as('row2');
      cy.get('@row2').eq(1).find('[data-cy="addButton"]').click();
      cy.get('@row2').eq(1).contains('My Project');
      cy.get('@row2').eq(1).find('[data-cy="removeBtn"]')
    });
    ...
    Cypress.Commands.add("removeFromMyProjectsViaBtn", (rowNum, expectedNumRows = 6) => {
      ...
      cy.get('@row2').eq(1).find('[data-cy="addButton"]')
    ...
    Cypress.Commands.add("containsCellValue", (rowNum, columnNum, expectedValue, numRows = 6) => {
      const tableSelector = '[data-cy="discoverProjectsTable"]';
      const rowSelector = `${tableSelector} tbody tr`;
      cy.get(rowSelector).should('have.length', numRows).as('cyRows');
      cy.get('@cyRows').eq(rowNum).find('td').as('row2');
      cy.get('@row2').eq(columnNum).contains(expectedValue);
    });
    ...
    Cypress.Commands.add("doesNotContainsCellValue", (rowNum, columnNum, expectedValue) => {
      ...
      cy.get('@cyRows').eq(rowNum).find('td').as('row2');
      cy.get('@row2').eq(columnNum).contains(expectedValue).should('not.exist');
    });
  });
  ...
  it('display stat cards', function () {
    ...
    cy.get('[data-cy="allProjectsCount"] [data-cy="mediaInfoCardTitle"]').contains('8');
    cy.get('[data-cy="myProjectCount"] [data-cy="mediaInfoCardTitle"]').contains('0');
    cy.get('[data-cy="discoverNewProjCount"] [data-cy="mediaInfoCardTitle"]').contains('8');
    ...
    cy.get('[data-cy="allProjectsCount"] [data-cy="mediaInfoCardTitle"]').contains('8');
    cy.get('[data-cy="myProjectCount"] [data-cy="mediaInfoCardTitle"]').contains('2');
    cy.get('[data-cy="discoverNewProjCount"] [data-cy="mediaInfoCardTitle"]').contains('6');
  });
  ...
  it('stat cards are updated in real time when projects are added and removed', function () {
    ...
    cy.get('[data-cy="myProjectCount"] [data-cy="mediaInfoCardTitle"]').contains('2');
    cy.get('[data-cy="discoverNewProjCount"] [data-cy="mediaInfoCardTitle"]').contains('7');

    cy.addToMyProjectsViaBtn(1)
    cy.get('[data-cy="allProjectsCount"] [data-cy="mediaInfoCardTitle"]').contains('9');
    cy.get('[data-cy="myProjectCount"] [data-cy="mediaInfoCardTitle"]').contains('3');
    cy.get('[data-cy="discoverNewProjCount"] [data-cy="mediaInfoCardTitle"]').contains('6');

    cy.addToMyProjectsViaBtn(4)
    cy.get('[data-cy="allProjectsCount"] [data-cy="mediaInfoCardTitle"]').contains('9');
    cy.get('[data-cy="myProjectCount"] [data-cy="mediaInfoCardTitle"]').contains('4');
    cy.get('[data-cy="discoverNewProjCount"] [data-cy="mediaInfoCardTitle"]').contains('5');
  ...
  it('search by name', function () {
    ...
    cy.get('[data-cy="projectsTableTotalRows"]').contains(9);

    cy.get('[data-cy="searchInput"]').type('eAT');
    ...
    cy.get('[data-cy="projectsTableTotalRows"]').contains(3);

    cy.get('[data-cy="searchInput"]').type('{backspace}{backspace}{backspace}');
    cy.containsCellValue(5, 0, 'This is project 6')
    cy.get('[data-cy="projectsTableTotalRows"]').contains(9);
    ...
    // clear search
    cy.get('[data-cy="clearSearch"]').click();
    cy.containsCellValue(5, 0, 'This is project 6')
    cy.get('[data-cy="projectsTableTotalRows"]').contains(9);
  });
  ...
  it('stats cards update when adding and removing projects after performing a search', function () {
    ...
    cy.get('[data-cy="searchInput"]').type('project 3');
    cy.containsCellValue(0, 0, 'This is project 3', 2)
    cy.containsCellValue(1, 0, 'this is project 3 fleat beat', 2)
    cy.get('[data-cy="projectsTableTotalRows"]').contains(2);


    cy.get('[data-cy="allProjectsCount"] [data-cy="mediaInfoCardTitle"]').contains('9');
    cy.get('[data-cy="myProjectCount"] [data-cy="mediaInfoCardTitle"]').contains('5');
    cy.get('[data-cy="discoverNewProjCount"] [data-cy="mediaInfoCardTitle"]').contains('4');

    cy.addToMyProjectsViaBtn(1, 2);
    cy.get('[data-cy="allProjectsCount"] [data-cy="mediaInfoCardTitle"]').contains('9');
    cy.get('[data-cy="myProjectCount"] [data-cy="mediaInfoCardTitle"]').contains('6');
    cy.get('[data-cy="discoverNewProjCount"] [data-cy="mediaInfoCardTitle"]').contains('3');
  ...
  it('search filters all the rows', function () {
    ...
    cy.get('[data-cy="searchInput"]').type('dljlajd');
    cy.get('[data-cy="projectsTableTotalRows"]').contains(0);
    cy.get('[data-cy="discoverProjectsTable"]').contains('There are no records');
    cy.get('[data-cy="discoverProjectsTable"]').contains('Please modify your search string: [dljlajd]');
