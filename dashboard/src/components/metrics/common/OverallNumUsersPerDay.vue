  ...
  if (appConfig) {
    ...
    if (!byMonth.value) {
      if (timeEvent.startTime < oldestDaily) {
        mutableTitle.value = 'Overall Users per week'
      } else {
        mutableTitle.value = props.title
      }
    } else {
      mutableTitle.value = 'Overall Users per month'
    }
  }
...
const loadData = () => {
  loading.value = true
  MetricsService.getOverallMetricsChart(
    'overallDistinctUsersOverTimeMetricsBuilder',
    localProps.value
  ).then((response) => {
...
<template>
  <Card
    data-cy="overallDistinctNumUsersOverTime"
    class="w-full"
    :style="`width: ${layoutSizes.tableMaxWidth}px;`">
    <template #header>
        ...
        <template #headerContent>
          <div class="flex gap-2 items-center">
            ...
            <time-length-selector
              ...
              @time-selected="updateTimeRange"
              ref="timeRangeSelector"
              :disable-days="byMonth" />
    ...
    <template #content>
      <metrics-overlay
        :loading="loading"
        :has-data="hasDataEnoughData"
        no-data-msg="This chart needs at least 2 days of user activity.">
        <Chart
          ref="usersChartRef"
          id="overallUsersTimeChart"
          type="line"
          :data="distinctUsersOverTimeChartData"
          :options="chartJsOptions"
          class="h-[30rem]" />
        <div
          v-if="usersChartAnimationCompleted"
          data-cy="overallUsersTimeChart_animationCompleted" />
      </metrics-overlay>
    </template>
  </Card>
</template>
