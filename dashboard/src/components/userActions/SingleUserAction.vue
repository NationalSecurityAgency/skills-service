...
<loading-container :is-loading="loading">
  <div v-if="attributes && attributes.length > 0">
    <div v-for="(attr) in attributes" :key="attr.label">
      <div v-if="attr.isDescription && attr.value">
        ...
        <div >
            <markdown-text v-if="attr.value" :text="attr.value" data-cy="descriptionText"/>
        </div>
        </b-card>
      </div>
      <div v-else class="row">
        ...
        <div class="col my-auto">
          ...
          <span v-else>
            <div v-if="attr.isTextAreaProp"><b-textarea :value="attr.value" :readonly="true"/></div>
      ...
      loadData() {
        ...
        UserActionsService.getDashboardSingleAction(this.actionId)
          .then((res) => {
            ...
            this.attributes = Object.entries(loadedObj).map((entry) => {
