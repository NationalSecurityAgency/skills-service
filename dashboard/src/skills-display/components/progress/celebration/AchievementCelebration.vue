...
const showCelebration = computed(() => dateAchieved.value && timeUtils.isWithinNDays(dateAchieved.value, 7))
const projectId = computed(() => attributes.projectId)
const storageStartKey = isSubject ? `celebration-proj-${projectId.value}-subject-${props.userProgress.subjectId}-achievement`: `celebration-proj-${projectId.value}-achievement`
const projectLevelCelebrationClosed = useStorage(`${storageStartKey}-level${level.value}-closed`, false)
const projectLevelCelebrationConfettiDisplayed = useStorage(`${storageStartKey}-level${level.value}-confetti-shown`, false)
...
onMounted(() => {
  if (showCelebration.value && !projectLevelCelebrationClosed.value && !projectLevelCelebrationConfettiDisplayed.value && !disableEncouragementsConfetti.value) {
    showConfetti()
    projectLevelCelebrationConfettiDisplayed.value = true
  }
})
...
const close = () => {
  projectLevelCelebrationClosed.value = true
}
...
<template>
  <Message v-if="showCelebration && !projectLevelCelebrationClosed" severity="success" data-cy="achievementCelebrationMsg">
    <template #container>
