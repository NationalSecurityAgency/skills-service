...
class CopySkillsToAnotherProjSubjValidationSpecs extends CopyIntSpec {
    ...
    def "validate that all provided skill ids exist"() {
        ...
        skillsService.copySkillDefsIntoAnotherProjectSubject(p1.projectId, skillIds, p2.projectId, p2subj1.subjectId)
        then:
        SkillsClientException ex = thrown(SkillsClientException)
    ...
    def "validate dest proj exist"() {
        ...
        skillsService.copySkillDefsIntoAnotherProjectSubject(p1.projectId, p1Subj1Skills.collect { it.skillId as String }, badProjectId, p2subj1.subjectId)
        then:
        SkillsClientException ex = thrown(SkillsClientException)
    ...
    def "validate subject exist"() {
        ...
        when:
        skillsService.copySkillDefsIntoAnotherProjectSubject(p1.projectId, p1Subj1Skills.collect { it.skillId as String }, p2.projectId, p2subj1.subjectId + "a")
        then:
        SkillsClientException ex = thrown(SkillsClientException)
        ex.message.contains("Subject with id [${p2subj1.subjectId + "a"}] does not exist")
    }
    ...
    def "validate subject id is provided"() {
        ...
        when:
        skillsService.copySkillDefsIntoAnotherProjectSubject(p1.projectId, p1Subj1Skills.collect { it.skillId as String }, p2.projectId, p2Subj1Skills[0].skillId)
        then:
        SkillsClientException ex = thrown(SkillsClientException)
        ex.message.contains("Provided id [${p2Subj1Skills[0].skillId}] is not for a subject")
    }
    ...
    def "user with approver role for the destination project does not have permission to copy"() {
        ...
        def p2 = createProject(2)
        def p2subj1 = createSubject(2, 2)
        otherUser.createProjectAndSubjectAndSkills(p2, p2subj1, [])
    ...
    def "must be an admin of destination project to copy"() {
        ...
        when:
        skillsService.copySkillDefsIntoAnotherProjectSubject(p1.projectId, p1Skills.collect { it.skillId as String }, p2.projectId, p2subj1.subjectId)
        then:
        SkillsClientException ex = thrown(SkillsClientException)
        ex.message.contains("User [${skillsService.userName}] is not an admin for destination project [${p2.projectId}]")
    }
    ...
    def "do not allow to copy from community protected project to a non-community protected project"() {
        ...
        def p2 = createProject(2)
        def p2subj1 = createSubject(2, 2)
        dragonUser.createProjectAndSubjectAndSkills(p2, p2subj1, [])

        when:
        dragonUser.copySkillDefsIntoAnotherProjectSubject(p1.projectId, p1Skills.collect { it.skillId as String }, p2.projectId, p2subj1.subjectId)
        then:
        SkillsClientException ex = thrown(SkillsClientException)
    ...
    def "do allow to copy imported skills that are not finalized"() {
        ...
        skillsService.createProjectAndSubjectAndSkills(projToCopy, p3subj1, [])

        when:
        List<String> skillIdsToCopy = [p1Skills[0].skillId, p1Skills[1].skillId]
        skillsService.copySkillDefsIntoAnotherProjectSubject(p2.projectId, skillIdsToCopy, projToCopy.projectId, p2subj1.subjectId)
        then:
        SkillsClientException ex = thrown(SkillsClientException)
    ...
    def "do not allow to copy reused skills"() {
        ...
        skillsService.copySkillDefsIntoAnotherProjectSubject(p1.projectId, skillIdsToCopy, p2.projectId, p2subj1.subjectId)
        then:
        SkillsClientException ex = thrown(SkillsClientException)
