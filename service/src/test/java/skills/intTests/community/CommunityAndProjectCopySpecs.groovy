...
class CommunityAndProjectCopySpecs extends DefaultIntSpec {
    ...
    def "prevent skill reuse if any of the skills have an invalid description "() {
        ...
        then:
        def exception = thrown(SkillsClientException)
        exception.message.contains("Failed to reuse a skill due to the paragraph validation")
        exception.message.contains("errorCode:ParagraphValidationFailed")
        exception.message.contains("skillId:${p1Skills[2].skillId}")
    }

    def "prevent skill reuse if any of the skills video transcript are invalid"() {
        List<String> users = getRandomUsers(2)
        SkillsService pristineDragonsUser = createService(users[1])
        SkillsService rootUser = createRootSkillService()
        rootUser.saveUserTag(pristineDragonsUser.userName, 'dragons', ['DivineDragon'])
    ...
    def "prevent skill reuse if any of the skills video transcript are invalid - internally hosted video"() {
        List<String> users = getRandomUsers(2)
        SkillsService pristineDragonsUser = createService(users[1])
        SkillsService rootUser = createRootSkillService()
        rootUser.saveUserTag(pristineDragonsUser.userName, 'dragons', ['DivineDragon'])
