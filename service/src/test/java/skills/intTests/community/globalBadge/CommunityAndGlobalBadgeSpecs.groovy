...
class CommunityAndGlobalBadgeSpecs extends DefaultIntSpec {
    ...
    def "cannot enable community for a project if it belongs to a non UC Global badge - skill assigned"() {
        ...
        pristineDragonsUser.assignProjectLevelToGlobalBadge(projectId: p2.projectId, badgeId: badge2.badgeId, level: "1")
    ...
    def "available skills endpoint for non UC badge does NOT return skills from UC protected projects"() {
        ...
        when:
        def res = pristineDragonsUser.getAvailableSkillsForGlobalBadge(badge1.badgeId, "")
        def res1 = pristineDragonsUser.getAvailableSkillsForGlobalBadge(badge1.badgeId, "2")

        def res2 = pristineDragonsUser.getAvailableProjectsForGlobalBadge(badge1.badgeId, "")
        def res3 = pristineDragonsUser.getAvailableProjectsForGlobalBadge(badge1.badgeId, "1")
        then:
    ...
    def "available skills endpoint for UC badge does NOT return skills from non UC protected projects"() {
        ...
        def res2 = pristineDragonsUser.getAvailableProjectsForGlobalBadge(badge1.badgeId, "")
        def res3 = pristineDragonsUser.getAvailableProjectsForGlobalBadge(badge1.badgeId, "2")
        then:
        res.suggestedSkills.skillId.sort() == p1Skills.skillId.sort()
        res.totalAvailable == 3

        res1.suggestedSkills.skillId.sort() == [p1Skills[1].skillId]
        res1.totalAvailable == 1

        res2.projects.projectId == [p1.projectId]
        res2.totalAvailable == 1
