...
class OpenAIApiConfig {
    ...
    @Value('#{"${skills.openai.embeddingsHost}"}')
    String aiEmbeddingsHost

    @Value('#{"${skills.openai.completionsEndpoint:/v1/chat/completions}"}')
    String completionsEndpoint

    @Value('#{"${skills.openai.embeddingsEndpoint:/v1/embeddings}"}')
    String embeddingsEndpoint

    @Bean("openAiApi")
    OpenAiApi openAiApi(WebClient.Builder webClientBuilder) {
        return crateOpenAiApi(webClientBuilder, aiHost)
    }

    @Bean("embeddingsOpenAiApi")
    OpenAiApi embeddingsOpenAiApi(WebClient.Builder webClientBuilder) {
        String host = aiEmbeddingsHost ?: aiHost
        log.info("Embeddings host: {}", host)
        return crateOpenAiApi(webClientBuilder, host)
    }

    private OpenAiApi crateOpenAiApi(WebClient.Builder webClientBuilder, String aiHost) {
        if (!aiHost) {
        ...
        OpenAiApi openAiApi = OpenAiApi.builder()
                ...
                .embeddingsPath(embeddingsEndpoint)
