...
@Component
class EmailVerificationNotificationBuilder implements NotificationEmailBuilder {

    private static final String VERIFY_EMAIL_TEMPLATE = "verify_email.html"
    ...
    @Override
    String getId() {
        Notification.Type.VerifyEmail.toString()
    }
    ...
    Res build(Notification notification, Formatting formatParams) {
        ...
        try {
            String htmlBody = thymeleafTemplateEngine.process(VERIFY_EMAIL_TEMPLATE, context)
    ...
    private String buildPlainText(Object parsed, Formatting formatting) {
        ...
        String pt = "Hi ${parsed.recipientName},\n" +
                "\n" +
                "\nWe're happy you created a SkillTree account! Please use the link below to confirm your email address so you can start exploring SkillTree. The link will be valid for ${parsed.validTime}.\n"
                "\n${parsed.publicUrl}verify-email/${parsed.token}/${parsed.email}" +
                "\n\n" +
                "\nRegards," +
