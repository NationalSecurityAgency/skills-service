...
@Component
class QuizAiGradingFailedNotificationBuilder implements NotificationEmailBuilder {
    ...
    @Override
    String getId() {
        return Notification.Type.TextInputAiGradingFailed.toString()
    }
    ...
    Res build(Notification notification, Formatting formatting) {
        ...
        String htmlBody = thymeleafTemplateEngine.process("quiz_ai_grading_failed.html", context)
        String plainText = buildPlainText(parsed, formatting)
        return new Res(
                subject: "SkillTree Quiz AI Grading Failed!",
                html: htmlBody,
                plainText: plainText,
    ...
    private static String buildPlainText(Object parsed, Formatting formatting) {
        String pt = "SkillTree Quiz AI Grading Failed!\n\n" +
                "The automated AI grading system for the [${parsed.quizName}] quiz encountered an error and could not complete the evaluation. This quiz now requires manual grading intervention. As a quiz administrator, please review the submitted responses and determine whether the user should pass or fail this quiz attempt.\n\n" +
