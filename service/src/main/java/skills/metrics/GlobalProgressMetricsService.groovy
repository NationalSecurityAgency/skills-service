...
class GlobalProgressMetricsService {
    ...
    GlobalMetricsResult loadMetrics(String userQuery, PageRequest pageRequest) {
        ...
        List<GlobalProgressMetricsRepo.QuizIdAndType> quizIdAndTypes = globalProgressMetricsRepo.getQuizTypes(quizIds)
        return new GlobalMetricsResult(
                ...
                numTotalQuizzes: quizIdAndTypes?.findAll {it.quizType == QuizDefParent.QuizType.Quiz}?.size() ?: 0,
                numTotalSurveys: quizIdAndTypes?.findAll {it.quizType == QuizDefParent.QuizType.Survey}?.size() ?: 0,
